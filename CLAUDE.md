...
| `/steer` | POST | `{reqId, text}` | `{ok, delivered}`, or 409 when that run is not taking messages. Writes a message into a running `/chat` job: claude gets it over the stream-json stdin, GPT runs over codex app-server `turn/steer`. |
...
- **GenMotion as an engine.** Once the app is installed, the composer's engine menu offers GenMotion next to Remotion and HyperFrames (owner-only, like the rest of that menu). A send on it never reaches `/chat`. It opens GenMotion (sharing the Premiere project folder when the project is saved), puts the prompt on the clipboard, and writes the prompt to History. The next export GenMotion finishes shows up in that chat as a normal render card, and the prompt-only History entry is replaced by the file. A panel reload forgets the waiting send, but the export is still in the GenMotion menu.
...
- `genmotion` on PATH is a 1.4KB shell launcher, not a CLI. It only runs `open -a GenMotion.app`, optionally with `--gm-cwd=<folder>` to share a folder. There is no prompt flag and no render command, and its `genmotion://` links only remix a template or finish sign-in.
...
- **The Remotion skill sync looks for `skills/remotion-best-practices`.** Remotion renamed `skills/remotion` and moved `rules/` into topic folders (`remotion-markup/`, `remotion-render/`, ...). The bridge only knew the old path, so every sync from July failed with "unexpected tarball layout" and the skill froze for 84 days. The sync now walks the whole skill and still never touches a hand-edited file, so the local `SKILL.md` and the custom rules (`motion-design.md`, `charts.md`, the `sfx.md` fix) stay. Renders still inline `rules/*.md` on purpose: the new upstream `timing.md` dropped the `spring()` section that `motion-design.md` is built on.
- **The owner's `remotion-intro` is a pnpm install** (isolated linker). Upgrade it with pnpm, passing each package as its own argument (zsh does not word-split `$VAR`). `npx remotion upgrade` picks npm while the stale `package-lock.json` is there and fails with "Invalid Version". Folders an old npm install left at the top of `node_modules/@remotion` are not managed by pnpm and go stale on upgrade; `npx remotion versions` catches that. Fresh installs from `bridge/remotion-template` use npm and don't have this problem.
- **Renders take their prompt over stdin, and stdin stays open.** `runClaudeOnce` spawns claude with `--input-format stream-json --include-partial-messages` and writes the prompt as a JSON user message, so `/steer` can write another one mid-run (Claude Code's steer). stdin must be closed at the first `result`, or claude waits for more input and the render never returns. A steer that lands after claude started its final answer runs as a second pass, so replies from every pass are joined to keep the first pass's import marker.
- **GPT runs go through `codex app-server`** (JSON-RPC over stdio: `initialize`, `thread/start`, `turn/start`, `turn/steer`), which streams reasoning summaries, commands and the reply. If it will not start, the run falls back to `codex exec`, which can neither stream nor steer.
- **The live feed is the `activity` SSE event** (`kind`: thinking, reasoning, say, writing, step, error). Claude's thinking text is not in the CLI stream (the block arrives empty), so for Claude the feed shows only that it is thinking and for how long. GPT's reasoning summaries arrive as text. Text and file-writing deltas are batched every 300ms.
- **The panel pauses its animation while unfocused and idle.** The manifest disables CEF's frame limit and vsync, so the particle loop is capped at 30fps by a timer, and `syncIdlePause` stops it plus every CSS animation (`html.panel-idle`) while the panel is unfocused and nothing is running. Idle cost went from 5.4% of a core to 0.1%.
