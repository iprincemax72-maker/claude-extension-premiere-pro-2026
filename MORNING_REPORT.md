...
- **Bug to review tomorrow: 5 Settings panel options are dead UI.** Audited every `userSettings.*` field. These five are written by Settings but **never read** anywhere in the panel or bridge:
  - **`pauseDur` + `noise`** (Auto-cut → Pause threshold, Silence sensitivity) — autocut POST body doesn't include them; bridge `/autocut` doesn't accept them; `detectSilences()` hardcodes `-35dB:d=0.7`.
  - **`transcript`** (Auto-cut → Detect fillers / false starts) — `findFillers` in the autocut POST body comes from `optFillers.checked` in the autocut modal, not from this setting. The Setting was probably intended to be the modal's pre-fill default.
  - **`audio`** (Output → Render audio in animations) — the bridge's `--mute`/stripAudioInPlace pipeline runs unconditionally; this toggle does nothing.
  - **`autoOpen`** (Output → Auto-open render in browser) — render-card preview/import buttons don't consult this setting; render results never auto-open.

  Wired settings (for reference): `accent`, `theme`, `placement`, `bootIntro`, `autoUpdate`, `renderMode`. `expand` is half-wired — the "Default expand level" setting is saved but the three Light/Medium/Heavy buttons in the composer don't pre-select based on it.

  Fix options for each dead setting: either (a) wire it through (panel body → bridge endpoint → behavior), or (b) rip the UI row. Held off on autonomous fixes because each one touches user-facing behavior. Your call in the morning. The wiring for `pauseDur`/`noise` would specifically be: (1) panel — add `pauseDur, noise` to the autocut fetch body at index.html ~4060; (2) bridge — accept those in payload and pass to `detectSilences()`; (3) `detectSilences` — interpolate them into the silencedetect args. Also bump SETTINGS_DEFAULTS pauseDur to `'0.7'` and noise to `'-35'` so wiring doesn't silently change current cut aggressiveness.
