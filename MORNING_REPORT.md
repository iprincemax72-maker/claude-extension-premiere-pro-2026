...
- **Bug to review tomorrow: Settings panel's Auto-cut options are dead UI.** The `Pause threshold` (0.4/0.6/1.0/1.5s) and `Silence sensitivity` (−40/−30/−25 dB) buttons in Settings → Auto-cut save to `userSettings.pauseDur` / `userSettings.noise`, but nothing reads those values when `/autocut` is called. The panel's autocut POST body (index.html line ~4060) doesn't include them; the bridge's `/autocut` endpoint doesn't accept them; `detectSilences()` uses hardcoded `-35dB:d=0.7`. To wire it up: (1) panel — add `pauseDur, noise` to the body at the autocut fetch; (2) bridge — accept those in payload and pass to `detectSilences()`; (3) `detectSilences` — interpolate them into the silencedetect args. Also worth updating the SETTINGS_DEFAULTS to match what the bridge actually uses today (`pauseDur: '0.7', noise: '-35'`) so wiring it up doesn't silently change current behavior. I held off because this touches user-facing autocut behavior — your call whether to wire or rip the dead UI.
