...
- [ ] Auto-Edit lower-third HEIGHT: first geometric fix jammed graphics to the very bottom edge (told Claude "bottom-anchored, anchor to bottom edge"). Reworked to a FLOATING lower-third: top bound (off face) + bottom bound (margin below, ~11% vertical) + explicit target CENTER (~77% vertical). Graphic now sits in the lower third with clear space below, not glued to the bottom. **Needs a live run** to confirm it matches the "a little up" target. Knobs in generateMomentsParallel: `targetCenterFrac`, `bottomMarginFrac`, `faceSafeTopFrac`.
